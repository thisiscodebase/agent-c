...
- `agent/lib/base-instructions.ts` — rewritten system prompt: an internal
  ...
  States plainly what it's for (case studies, reports, cross-source lookup)
  and what it isn't (not a coding agent, not a replacement for
  Drive/HubSpot/Notion/Slack's own search).
...
- `agent/tools/github.ts` + GitHub Integrations registry entry — not a
  CodeBase lookup source (coding-agent/PR work is a separate future project).
...
- `agent/skills/daily-summary.md` — kept, rewritten as the case-study-
  ...
  rather than replace. Full digest rework still depends on Phase 4 artifacts.
...
- Slack channel retained from upstream (`agent/channels/slack.ts`); search
  uses the **same** Connect app with expanded scopes via `search_slack.ts`.
...
- `artifacts`
- `artifact_sources` (designed `source_type`: `drive | hubspot | notion | slack`)
...
- **Drive**: Google official Drive MCP (Developer Preview), not OpenAPI
  against the Drive REST API.
