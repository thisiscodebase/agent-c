...
other connectors (Drive, HubSpot, Notion, Tally, Asana, Retool, Slack) via Vercel Connect /
OIDC in production.
...
| **Retool** | Per-user OAuth | Users connect in Integrations; scopes `mcp:read` + `mcp:write` |
...
2. Connect / Test each connector (Drive, HubSpot, Notion, Tally, Asana, Retool, Slack).
3. Chat smoke: Drive file search, HubSpot company, Notion page, Tally form,
   Asana tasks, Retool apps/resources, Slack search, Platform company list.
...
| Drive/HubSpot/Notion/Tally/Asana/Retool/Slack | — | via Connect UIDs | Connect create/attach + user OAuth where per-user |
