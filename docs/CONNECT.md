...
Why 🍊 Agent C uses [Vercel Connect](https://vercel.com/docs/connect), what it
does versus MCP servers, how to provision it for demos, what a DIY replacement
would entail, and realistic pricing at CodeBase scale.
...
| -------------- | -------------------------------------------------- | ------------------------------------------------------------------------------ |
| **MCP server** | Remote tool host — schemas, search/read/write APIs | `drivemcp.googleapis.com`, `mcp.hubspot.com`, `mcp.notion.com`, `api.tally.so` |
...
## What Connect does in 🍊 Agent C

| Capability               | Where it lives                                                                  |
| ------------------------ | ------------------------------------------------------------------------------- |
| Per-user OAuth           | Drive, Notion, Tally, Slack search (`server/utils/connect.ts`, Integrations UI) |
...
Connect does **not** host MCP servers. Google, HubSpot, and Notion host those.
Connect brokers credentials so Eve never sees long-lived secrets in env vars or
model context.
...
You also need normal app env (auth, database, `AI_GATEWAY_API_KEY` or OIDC). See
...
Connect bills **per token request** — each `getToken` / `getTokenResponse` call
to the Connect API (not per user, not per connector).
...
Eve caches tokens **per workflow step**, so multiple tool calls in the same step
for the same provider do not each hit Connect.
...
less. Annual run-rate **~$60–180** unless usage is much heavier than assumed
(deep multi-connector research loops, constant Integrations testing).

Connect cost is usually **negligible vs engineering time** to replace it at this
scale.
...
- HubSpot as a single env token (loses per-user CRM attribution) — small savings
  only.
...
- **MCP OAuth only** (Drive, Notion, HubSpot, Slack search): ~**2–3 weeks** for
  a solid v1.
- **+ Slack channel parity**: +**3–7 days**.
- **Ongoing**: provider API/OAuth changes, security reviews, on-call for broken
  grants.

A scrappy v1 in ~10–12 dev-days is possible with rough edges; true parity with
current Integrations + in-chat UX is closer to **3–4 weeks**.
...
- Connect pricing or limits grow materially with scale (hundreds of heavy users,
  very high connector churn).
