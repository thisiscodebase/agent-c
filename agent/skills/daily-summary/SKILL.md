...
2. **Gather live signal in parallel** (soft budget **≤4 tool steps**; prefer parallel). Choose sources from focus + role — do **not** spray every connector:
   ...
   | HubSpot | Focus includes deals, companies, or CRM work | `connection_search` once for HubSpot if tools are unknown, then search companies/deals/contacts matching focus names. Prefer recently updated. Never blank-query notes/emails. |
...
4. **Reply in chat** with this structure (omit empty sections):
   ...
   - **Moving** — CRM / Platform changes that matter (deal movement, sessions)
